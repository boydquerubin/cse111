...
# exceeding requirements, sum_protons function, also added the atomic number in dictionary
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    for symbol, quantity in symbol_quantity_list:
        ...
        max_protons += atomic_number * quantity
    return max_protons

def main():
    formula = input("What is the chemical formula for a molecule? ")
    mass = float(input("What is the chemical sample in grams? "))
    ...
    max_protons = sum_protons(symbol_quantity_list, periodic_table)

    print(f"Molar mass: {molar_mass:.5f} grams/mole")
    print(f"Number of moles: {moles:.5f} moles")
    print(f"Total number of protons: {max_protons}")
